...
class AdaIN_net(nn.Module):
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        if self.training:  # training
            ...
            # calculate Eq. (9)
            # f(c) and f(s) are encoded content and style, respectively
            content_feat = self.encode(content)[-1]
            style_feats = self.encode(style)
            t = self.adain(content_feat, style_feats[-1])  # relu 4_1
            # adjust the level of style transfer
            ...
            #   your code here ...
            content_feat = self.encode(content)[-1]
            style_feat = self.encode(style)[-1]
            t = self.adain(content_feat, style_feat)  # relu 4_1
            feat = alpha * t + (1 - alpha) * content_feat
